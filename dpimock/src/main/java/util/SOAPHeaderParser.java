...
import com.example.dpimock.OxalisAs4Exception;
...
public class SOAPHeaderParser {
    ...
    public static byte[] getSignature(SOAPMessage header) throws OxalisAs4Exception, SOAPException {

        NodeList sigNode = header.getSOAPBody().getElementsByTagNameNS(NS_ALL, SIG);
        if (sigNode.getLength() != 1) {
            throw new OxalisAs4Exception("Zero or multiple Signature elements in header");
        }
        Element sigElement = (Element) sigNode.item(0);
    ...
    public static List<ReferenceType> getReferenceListFromSignedInfo(SOAPHeader header) throws OxalisAs4Exception, SOAPException {
        NodeList sigInfoNode = header.getElementsByTagNameNS(NS_ALL, SIG_INFO);
        if (sigInfoNode.getLength() != 1) {
            throw new OxalisAs4Exception("Zero or multiple Signature elements in header");
        }
    ...
    public static UserMessage getUserMessage(SOAPHeader header) throws OxalisAs4Exception {
        ...
        try {
            ...
            JAXBElement<Messaging> unmarshal = unmarshaller.unmarshal(messagingNode, Messaging.class);

            messaging = unmarshal.getValue();
